...
use crate::{FilterCondition, key_segment::KeySegment};
...
pub struct RowOptionSet {
    ...
    pub filter_rules: Option<FilterCondition>,
    /// Dot-separated output paths to remove from the *final* row shape (after
    /// `KeySegment` placement), e.g. `["size", "depth"]` or `["addresses", "$",
    /// "admin2"]` -- `"$"` recurses into every element of an array instead of matching
    /// a literal key. Independent of which source column produced the nested
    /// structure; see `exclude_nested_path`. Unmatched/malformed paths are a no-op.
    pub excluded_paths: Vec<Vec<String>>,
}
...
impl RowOptionSet {
    ...
    // lets you set all options, but not filter rules
    ...
    // lets you set all options and filter rules
    pub fn new_with_rules(cols: &[Column], rules: &FilterCondition, decimal_comma: bool, datetime_mode: DateTimeMode) -> Self {
        RowOptionSet {
            decimal_comma,
            datetime_mode,
            omit_null_values: false,
    ...
    // Set rules in fluent builder style
    pub fn set_rules(&mut self, condition: FilterCondition) {
        self.filter_rules = Some(condition);
    }
    ...
    pub fn decimal_separator(&self) -> String {
        if self.decimal_comma { "," } else { "." }.to_string()
    }
}
...
impl OptionSet {
    ...
    pub fn row_mode(&self) -> String {
        if self.jsonl { "JSON lines" } else { "JSON" }.to_string()
    }
    ...
    pub fn to_json(&self) -> Value {
        ...
        output.insert(
            ...
            self.rows.decimal_separator().into(),
        );
    ...
    pub fn to_lines(&self) -> Vec<String> {
        ...
        if let Some(s_names) = self.selected.clone() {
            let plural = if s_names.len() > 1 { "s" } else { "" };
        ...
        lines.extend(vec![
            ...
            format!(
                ...
                self.header_row
                    .map(|v| v.to_string())
                    .unwrap_or_else(|| if self.detect_header {
            ...
            format!(
                ...
                self.data_row_index
                    .map(|v| v.to_string())
                    .unwrap_or_else(|| "default (immediately after header)".to_string())
            ),
    ...
    pub fn first_data_row_index(&self) -> Option<usize> {
        if self.header_row.is_none()
            && self.data_row_index.is_none()
            && self.effective_header_row_span() <= 1
        {
            return None;
        }
        let header_row_index = self.header_row_index();
        let default_start = if self.omit_header {
            header_row_index
        } else {
            header_row_index + self.effective_header_row_span()
        };
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let result = match self {
            ...
            Self::TruthyCustom(rules) => {
                let true_str: Vec<String> = rules
                    .true_options()
                    .iter()
                    .map(|o| o.pattern().to_string())
                    .collect();
                let false_str: Vec<String> = rules
                    .false_options()
                    .iter()
                    .map(|o| o.pattern().to_string())
                    .collect();
...
impl Column {
    ...
    pub fn from_source_key_with_format(
        source_key: &str,
        key_opt: Option<&str>,
        format: Format,
        ...
        let mut col =
            Self::from_key_ref_with_format(key_opt, format, default, datetime_mode, decimal_comma);
    ...
    pub fn from_json(json: &Value) -> Self {
        ...
        let fmt = match json.get("format").and_then(|v| v.as_str()) {
            Some(fmt_str) => match Format::from_str(fmt_str) {
                Ok(fmt) => fmt,
                Err(_) => Format::Auto,
            },
            None => Format::Auto,
        ...
        let mut col = if let Some(src) = source_key {
            Column::from_source_key_with_format(
                src,
                key_opt,
                fmt,
                default,
    ...
    pub fn from_key_ref_with_format(
        key_opt: Option<&str>,
        format: Format,
        default: Option<Value>,
        datetime_mode: DateTimeMode,
...
impl Extension {
    ...
    pub fn use_calamine(&self) -> bool {
        matches!(
            self,
            Self::Ods | Self::Xlsx | Self::Xlsm | Self::Xlsb | Self::Xls
        )
    }
    ...
    pub fn use_csv(&self) -> bool {
        matches!(self, Self::Csv | Self::Tsv | Self::SemicolonSeparatedValues)
    }
}
...
impl ReadMode {
    pub fn from_key(key: &str) -> Self {
        ...
        match sample.as_str() {
            ...
            "preview" | "p" | "pre" | "multimode" | "multiple" | "previewmultiple"
            | "previewmulti" | "m" => ReadMode::PreviewMultiple,
            _ => ReadMode::Sync,
        }
    }
...
mod tests {
    ...
    fn test_format_from_str_array_strips_a_js_like_quoted_separator() {
        assert_array_format(Format::from_str("text[].split(',')").unwrap(), "text", ",");
        assert_array_format(
            Format::from_str(r#"text[].split(",")"#).unwrap(),
            "text",
            ",",
        );
